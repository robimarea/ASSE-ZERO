...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const handleDeviceOrientation = useCallback((event: DeviceOrientationEvent) => {
    ...
    const centerX = shell.clientWidth / 2, centerY = shell.clientHeight / 2;
  ...
  useEffect(() => {
    ...
    const shell = shellRef.current;
    if (!shell) return;
    const pmh = handlePointerMove as EventListener;
    const peh = handlePointerEnter as EventListener;
    const plh = handlePointerLeave as EventListener;
    const doh = handleDeviceOrientation as EventListener;
    ...
    const handleClick = () => {
      ...
      if (anyMotion && typeof anyMotion.requestPermission === 'function') {
        anyMotion.requestPermission().then((state: string) => { if (state === 'granted') window.addEventListener('deviceorientation', doh); }).catch(console.error);
      } else { window.addEventListener('deviceorientation', doh); }
    };
  ...
  const cardStyle = useMemo(() => ({
    ...
    '--behind-glow-color': behindGlowColor ?? 'rgba(233, 172, 6, 0.67)',
